...
export const getOfficerDashboard = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.log(`Officer dashboard requested by: ${officerId}`);
      ...
      try {
        ...
        myFiles = myFilesSnap.docs.map((doc) => {
          const d = doc.data();
          return {
            id: doc.id,
            fileId: doc.id,
        ...
        console.log(`Found ${myFiles.length} files for officer ${officerId}`);
      } catch (err) {
        console.error("Error fetching my files:", err);
      }

      // ── 2. Escalated Files ─────────────────────────────────────
      let escalatedFiles: any[] = [];
      try {
        ...
        escalatedFiles = escalatedSnap.docs.map((doc) => {
          const d = doc.data();
          return {
            id: doc.id,
            fileId: doc.id,
        ...
        console.error("Error fetching escalated files:", err);
      }

      // ── 3. Decision Files (APPROVED / REJECTED by officer) ─────
      let decisionFiles: any[] = [];
      try {
        ...
        decisionFiles = approvedSnap.docs.map((doc) => {
          const d = doc.data();
        ...
        console.error("Error fetching decision files:", err);
      }
      ...
      try {
        ...
        qrLogs = qrLogsSnap.docs.map((doc) => {
          const d = doc.data();
      ...
      try {
        ...
        console.error("Error fetching movement counts:", err);
      }
      ...
      try {
        ...
        stats.slaBreaches = breachedSla;
      } catch (err) {
        console.error("Error fetching SLA stats:", err);
      }

      console.log(`Returning ${myFiles.length} myFiles, ${escalatedFiles.length} escalated, ${decisionFiles.length} decisions`);
